...
CONS_BCCC <- "ensemble_bccc/BCCC_consensus_matrix.rds"
OUT_DIR   <- "BCCC_output"
...
CM_BCCC <- readRDS(CONS_BCCC)

# Переводимо всі консенсуси у [0,1]
CM_BCCC <- normalize_matrix01(CM_BCCC)

CM_BCCC <- assign_dimnames_from_X(CM_BCCC, X, "BCCC")

S <- CM_BCCC
cat(sprintf("S (BCCC) range: [%.4f, %.4f]\n", min(S), max(S)))
...
max_k       <- 150
target_genes <- 15000
lambda <- 1.2    # штраф за відхилення від цільового U
mu     <- 0.6    # штраф за низьке покриття

tau_quantiles <- c(0.75, 0.80, 0.85, 0.90)
...
# збереження таблиць
f_sum <- file.path(OUT_DIR, "BCCC_best_summary_on_X.csv")
f_per <- file.path(OUT_DIR, "BCCC_best_biclusters_on_X.csv")
...
do_enrichment_hsa <- function(genes_entrez,
  ...
  if (!is.null(go_all_df) && nrow(go_all_df)) {
    write.csv(go_all_df, file.path(out_dir, "BCCC_enrichment_GO_all_BP_CC_MF.csv"),
              row.names = FALSE)
  }
...
run_kegg_with_plots <- function(genes_entrez,
  ...
  write.csv(df_all, file.path(out_dir, "BCCC_KEGG_enrichment_full.csv"), row.names = FALSE)
  
  # Top-N терміни за p.adjust
  df_top <- df_all[order(df_all$p.adjust), ][1:min(top_n, nrow(df_all)), ]
  write.csv(df_top, file.path(out_dir, sprintf("BCCC_KEGG_enrichment_top%d.csv", top_n)),
            row.names = FALSE)
  
  # Dotplot (clusterProfiler)
  suppressPackageStartupMessages(library(enrichplot))
  p <- dotplot(ekk, showCategory = top_n) +
    ggplot2::ggtitle(sprintf("BCCC: KEGG Pathway Enrichment (Top %d)", top_n)) +
    ggplot2::theme_minimal()
  
  # Збереження графіків
  ggplot2::ggsave(file.path(out_dir, sprintf("BCCC_KEGG_dotplot_top%d.png", top_n)),
                  plot = p, width = 8, height = 6, dpi = 300)
  ggplot2::ggsave(file.path(out_dir, sprintf("BCCC_KEGG_dotplot_top%d.pdf", top_n)),
...
write.csv(df_transform, "BCCC_subset_unique_genes.csv", row.names = FALSE)
