...
CONS_BCCC <- "BCCC_consensus_matrix.rds"
...
masks <- list(BCCC=CM_BCCC, ISA=CM_ISA, PMD=CM_PMD)
...
# збереження таблиць
f_sum <- file.path(OUT_DIR, "STACK_best_summary_on_X.csv")
f_per <- file.path(OUT_DIR, "STACK_best_biclusters_on_X.csv")
...
run_kegg_with_plots <- function(genes_entrez,
  ...
  write.csv(df_all, file.path(out_dir, "STACK_KEGG_enrichment_full.csv"), row.names = FALSE)
  
  # Top-N терміни за p.adjust
  df_top <- df_all[order(df_all$p.adjust), ][1:min(top_n, nrow(df_all)), ]
  write.csv(df_top, file.path(out_dir, sprintf("STACK_KEGG_enrichment_top%d.csv", top_n)),
            row.names = FALSE)
  ...
  # Збереження графіків
  ggplot2::ggsave(file.path(out_dir, sprintf("STACK_KEGG_dotplot_top%d.png", top_n)),
                  plot = p, width = 8, height = 6, dpi = 300)
  ggplot2::ggsave(file.path(out_dir, sprintf("STACK_KEGG_dotplot_top%d.pdf", top_n)),
...
write.csv(df_transform, "STACK_subset_unique_genes.csv", row.names = FALSE)
