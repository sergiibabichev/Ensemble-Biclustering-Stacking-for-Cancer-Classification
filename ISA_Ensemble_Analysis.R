...
CONS_ISA <- "isa_ensemble_full_outputs/isa_consensus_mask_full.rds"
OUT_DIR   <- "ISA_output"
...
max_k       <- 150
target_genes <- 14000
lambda <- 1.6    # штраф за відхилення від цільового U
mu     <- 0.6    # штраф за низьке покриття

nz <- S2 > 0
if (!any(nz)) stop("All S2 zeros for ISA.")

tau_quantiles <- c(0.90, 0.95, 0.975, 0.99)   # більш «хвостаті»
tau_grid <- as.numeric(quantile(S2[nz], probs = tau_quantiles, na.rm = TRUE))
...
# збереження таблиць
f_sum <- file.path(OUT_DIR, "ISA_best_summary_on_X.csv")
f_per <- file.path(OUT_DIR, "ISA_best_biclusters_on_X.csv")
...
do_enrichment_hsa <- function(genes_entrez,
  ...
  if (!is.null(go_all_df) && nrow(go_all_df)) {
    write.csv(go_all_df, file.path(out_dir, "ISA_enrichment_GO_all_BP_CC_MF.csv"),
              row.names = FALSE)
  }
...
run_kegg_with_plots <- function(genes_entrez,
  ...
  write.csv(df_all, file.path(out_dir, "ISA_KEGG_enrichment_full.csv"), row.names = FALSE)
  
  # Top-N терміни за p.adjust
  df_top <- df_all[order(df_all$p.adjust), ][1:min(top_n, nrow(df_all)), ]
  write.csv(df_top, file.path(out_dir, sprintf("ISA_KEGG_enrichment_top%d.csv", top_n)),
            row.names = FALSE)
  
  # Dotplot (clusterProfiler)
  suppressPackageStartupMessages(library(enrichplot))
  p <- dotplot(ekk, showCategory = top_n) +
    ggplot2::ggtitle(sprintf("ISA: KEGG Pathway Enrichment (Top %d)", top_n)) +
    ggplot2::theme_minimal()
  
  # Збереження графіків
  ggplot2::ggsave(file.path(out_dir, sprintf("ISA_KEGG_dotplot_top%d.png", top_n)),
                  plot = p, width = 8, height = 6, dpi = 300)
  ggplot2::ggsave(file.path(out_dir, sprintf("ISA_KEGG_dotplot_top%d.pdf", top_n)),
...
write.csv(df_transform, "ISA_subset_unique_genes.csv", row.names = FALSE)
